...
- [x] P9. Evidence-based alert copy and ntfy priority by reason: alert body states the arithmetic (price, count of readings, prior floor); mistake maps to max priority, threshold high, others default. Rationale: trust on the lock screen comes from visible evidence, and triage from priority. Done: FiredSignal carries evidence (readings count, prior floor), the body states the firing arithmetic plus a may-not-be-honoured caution on mistake fares per the Going research, and ntfy priority maps mistake to max, threshold to high, statistical reasons to default. Code verified; a live alert requires a real trigger and API spend.
...
- [x] P13. Quiet hours with mistake override: hold threshold, percentile, and drop alerts overnight for morning delivery; mistake fares always break through. Rationale: fatigue control without missing the fares that die in minutes. Done: QUIET_HOURS env ("22-7", server-local, unset disables) holds the routine reasons in dispatch; a held alert is not recorded so the first poll after the window re-detects and delivers only if the fare still qualifies, which is the honest reading of morning delivery (announcing a price that died overnight would be inventing a deal). Settings states the configured window or "not set". Mistake fares bypass the hold.
- [ ] [proposed] P14. Mutation endpoint auth: every /api mutation route (trips, items, watches, days) runs on the service role with no session or ownership check, safe only while the app is a single local account. Gate them behind real auth when sign-in ships. Rationale: debrief 2 flagged the pattern app-wide; RLS already protects reads, writes deserve the same once a second user can exist.
